...
kubectl apply -f deployment-hello-world-service.yaml
sleep 4
kubectl apply -f deployment-hello-world-db.yaml
...
PODNAME=$(kubectl get pods | grep hello-world-service | awk '{print $1}' | head -n 1)
echo $PODNAME


#Inside the Pod, let's read the enviroment variables from our container
#Notice the service information is there but not the db information. Since db wasn't defined when the Pod started.
kubectl exec -it $PODNAME -- /bin/sh 
    printenv | sort
exit


#If you delete the pod and it gets recreated, you will get the variables for the service and db service information.
...
kubectl delete -f deployment-hello-world-db.yaml
#kubectl delete service hello-world-db

#The enviroment variables stick around...to get a new set, the pod needs to be recreated.
kubectl exec -it $PODNAME -- /bin/sh -c "printenv | sort"

#Let's clean up after our demo
kubectl delete -f deployment-hello-world-service.yaml
